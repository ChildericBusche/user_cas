	...
	private function connect() {
		...
		if (!$this->connected) {
			...
			if(is_null($fs)) {
				...
				$fs       = new \OCA\user_ldap\lib\FilesystemHelper();
				$log      = new \OCA\user_ldap\lib\LogWrapper();
				$avatarM  = \OC::$server->getAvatarManager();
				$db       = \OC::$server->getDatabaseConnection();
				$userMap  = new \OCA\user_ldap\Mapping\UserMapping($db);
				$groupMap = new \OCA\user_ldap\Mapping\GroupMapping($db);
			...
			$this->access->setUserMapper($userMap);
			$this->access->setGroupMapper($groupMap);
